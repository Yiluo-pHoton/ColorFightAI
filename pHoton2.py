    ...
    while True:
        ...
        while not have_attacked and (cur_x < g.width and cur_y < g.height):
            ...
            if not this_cell.owner == g.uid:
                for i in direct_dirs:
                    ...
                    if not sur_c == None:
                        if sur_c.owner == g.uid:
                            ...
                            this_cell_val -= (sur_c.takeTime // 4)
                ...
                if this_cell.takeTime > 5:
                    this_cell_val -= 4
                elif this_cell.takeTime <= 5:
                    this_cell_val -= 1
        ...
        now = datetime.datetime.now()
        timedelta = (now - last_attack_time).total_seconds()
        if not have_attacked and (now - last_attack_time).total_seconds() > 2.0 and (highest_val_coor) != last_attack_cell:
                print(g.AttackCell(highest_val_coor[0], highest_val_coor[1]), "x-value", highest_val_coor[0], "y-value", highest_val_coor[1])
                last_attack_time = datetime.datetime.now()
                print("last attacked:", last_attack_cell)
